<template>
  ...
  <EdgeLabelRenderer>
    ...
    >
      ...
      >
        <button 
          class="flex items-center gap-1 text-xs px-2 py-1 rounded-full bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-600 shadow-sm hover:shadow transition-shadow"
        >
          {{ currentRoleLabel }}
...
const imageRoleOptions = computed(() => [
  { label: t('node.first_frame'), key: 'first_frame_image' },
  { label: t('node.last_frame'), key: 'last_frame_image' },
  { label: t('node.ref_images'), key: 'input_reference' }
])
...
const currentRoleLabel = computed(() => {
  ...
  return option?.label || t('node.first_frame')
})
