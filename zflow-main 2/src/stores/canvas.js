  ...
  switch (type) {
    ...
    case 'video':
      return {
        url: '',
        duration: 0,
        label: 'node.video_node'
      }
    case 'image':
      return {
        url: '',
        label: 'node.image_node'
      }
    default:
      return {}
  }
...
export const loadProject = (projectId) => {
  ...
  if (canvasData) {
    // Restore nodes | 恢复节点
    // Filter out invalid nodes to prevent render errors | 过滤无效节点以防止渲染错误
    nodes.value = (canvasData.nodes || [])
      .filter(n => n && n.id && n.type && n.position)
      .map(n => ({
        ...n,
        data: {
          ...getDefaultNodeData(n.type), // Merge with defaults to ensure all fields exist | 合并默认值确保所有字段存在
          ...(n.data || {})
        }
      }))
...
export const undo = () => {
  if (historyIndex.value <= 0) {
    window.$message?.info(i18n.global.t('node.no_undo'))
    return false
  }
...
export const redo = () => {
  if (historyIndex.value >= history.value.length - 1) {
    window.$message?.info(i18n.global.t('node.no_redo'))
    return false
  }
